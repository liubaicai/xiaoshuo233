...
const logger = require('./logger')('request-promise-retry');
...
class rpRetry {
    static _rpRetry(options) {
        ...
        const fetchDataWithRetry = tryCount => {
            return requestPromise(options)
                ...
                .catch(err => {
                    logger.error(`Encountered error ${err.message} for ${options.method} request to ${options.uri}, retry count ${tryCount}`);
                    tryCount -= 1;
                    if (tryCount) {
    ...
    static _rp(options) {
        ...
        return requestPromise(options)
            ...
            .catch(err => {
                logger.error(`Encountered error ${err.message} for ${options.method} request to ${options.uri}`);
                return Promise.reject(err);
            });
    }
